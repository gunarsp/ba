...
class Payments extends Admin_Controller {
	...
	public function index() {
		...
		switch ($order_by) {
			case 'payment_id':
				$params['order_by'] = 'ba_payments.payment_id ' . $order;
				break;
			case 'client':
		...
		if (!$this->session->userdata('global_admin')) {
			...
			$params['where'][] = 'ba_invoices.invoice_id IN (SELECT invoice_id FROM ba_invoices WHERE user_id = ' . $this->session->userdata('user_id') . ')';

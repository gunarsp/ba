...
class Payment_Handler extends MX_Controller {
	...
	public function merchant_response() {
		...
		if (isset($_POST)) {
			...
			$this->merchant->driver = strtolower($this->mdl_ba_data->get('merchant_driver'));
			...
			if ($this->merchant->notify()) {
				...
				$payment_amount = $this->db->get('ba_merchant_responses')->row()->merchant_response_amount;
				...
				$this->db->insert('ba_payments', $db_array);
				...
				$this->db->where('merchant_response_id', $merchant_response_id);
				$this->db->update('ba_merchant_responses', $db_array);

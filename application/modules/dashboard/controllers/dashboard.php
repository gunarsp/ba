...
class Dashboard extends Admin_Controller {
	...
	function __construct() {
		...
		if ($this->mdl_ba_data->setting('dashboard_override')) {

			redirect($this->mdl_ba_data->setting('dashboard_override'));
	...
	function index() {
		...
		$module_upgrade_notice = $this->mdl_ba_modules->module_upgrade_notice();

		$data = array(
			'static_error'			=>	$module_upgrade_notice
		);

		if ($this->mdl_ba_data->setting('dashboard_show_open_invoices') == 'TRUE') {

			$data['open_invoices'] = $this->mdl_invoices->get_recent_open();

		}

		if ($this->mdl_ba_data->setting('dashboard_show_pending_invoices') == 'TRUE') {

			$data['pending_invoices'] = $this->mdl_invoices->get_recent_pending();

		}

		if ($this->mdl_ba_data->setting('dashboard_show_closed_invoices') == 'TRUE') {

			$data['closed_invoices'] = $this->mdl_invoices->get_recent_closed();

		}

		if ($this->mdl_ba_data->setting('dashboard_show_overdue_invoices') == 'TRUE') {

			$data['overdue_invoices'] = $this->mdl_invoices->get_recent_overdue();

		}

		if ($this->mdl_ba_data->setting('dashboard_show_quotes') == 'TRUE') {
	...
	function show_custom_menu() {
		...
		foreach ($this->mdl_ba_modules->custom_modules as $module) {

...
class Mdl_Inventory extends MY_Model {
	...
	public function __construct() {
		...
		$this->table_name = 'ba_inventory';

		$this->primary_key = 'ba_inventory.inventory_id';

		$this->select_fields = 'SQL_CALC_FOUND_ROWS ba_inventory.*, ba_inventory_types.*,
            IFNULL((SELECT SUM(inventory_stock_quantity) FROM ba_inventory_stock WHERE ba_inventory_stock.inventory_id = ba_inventory.inventory_id), 0) AS inventory_stock';

        $this->joins = array(
            'ba_inventory_types'   =>  array(
                'ba_inventory_types.inventory_type_id = ba_inventory.inventory_type_id', 'left'
            )
        );

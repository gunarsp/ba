...
class Mdl_Invoice_Reports extends MY_Model {
	...
	public function invoice_params() {
		...
		$params = array(
			'where'	=> array(
				'ba_invoices.invoice_date_entered >='	=>	strtotime(standardize_date($this->input->post('from_date'))),
				'ba_invoices.invoice_date_entered <='	=>	strtotime(standardize_date($this->input->post('to_date')))
			)
		);

		if ($this->input->post('client_id') <> 'all') {

			$params['where']['ba_invoices.client_id'] = $this->input->post('client_id');

		}

		if ($this->input->post('status_select') == 'open') {

			$params['where']['ba_invoices.invoice_status_id'] = 1;

		}

		elseif ($this->input->post('status_select') == 'pending') {

			$params['where']['ba_invoices.invoice_status_id'] = 2;

		}

		elseif ($this->input->post('status_select') == 'closed') {

			$params['where']['ba_invoices.invoice_status_id'] = 3;

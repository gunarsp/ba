...
class Mdl_Items extends MY_Model {
	...
	public function __construct() {
		...
		$this->table_name = 'ba_invoice_items';

		$this->primary_key = 'ba_invoice_items.invoice_item_id';

		$this->select_fields = "
		SQL_CALC_FOUND_ROWS *,
        ba_invoice_items.tax_rate_id AS item_tax_rate_id,
        ba_inventory.inventory_tax_rate_id AS inventory_tax_rate_id";
	...
	public function save($db_array, $invoice_item_id = NULL) {
		...
		if ($this->input->post('save_as_inventory') and $db_array['inventory_id'] == 'please_select') {
			...
			$this->db->insert('ba_inventory', $db_array);
	...
	public function delete($invoice_item_id) {
		...
		$this->db->delete(array('ba_invoice_items', 'ba_invoice_item_amounts'));

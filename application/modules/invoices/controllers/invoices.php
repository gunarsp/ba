...
class Invoices extends Admin_Controller {
	...
	function index() {
		...
		$params['where']['ba_invoices.invoice_is_quote'] = ($is_quote) ? 1 : 0;

		if (!$this->session->userdata('global_admin')) $params['where']['ba_invoices.user_id'] = $this->session->userdata('user_id');

		if ($client_id) {

			$params['where']['ba_invoices.client_id'] = $client_id;
		...
		switch ($order_by) {
			case 'invoice_id':
				$params['order_by'] = 'ba_invoices.invoice_number ' . $order;
				break;
			case 'client':
			...
			case 'amount':
				...
				break;
			case 'duedate':
				$params['order_by'] = 'ba_invoices.invoice_due_date ' . $order;
				break;
			case 'date':
				$params['order_by'] = 'ba_invoices.invoice_date_entered ' . $order;
				break;
			default:
				$params['order_by'] = 'ba_invoices.invoice_date_entered DESC, ba_invoices.invoice_number DESC';
		}
	...
	function edit() {
		...
		$params = array(
			'where'	=>	array(
				'ba_invoices.invoice_id'	=>	uri_assoc('invoice_id')
			)
		);
		...
		$data = array(
			...
			'document_incoming' =>	$this->mdl_classifiers->getDocumentDirections(),
		);
		//print_r($data);
	...
	function jquery_client_invoice_template($client_id, $quote = 0) {
		...
		$this->load->model('ba_data/mdl_ba_client_data');

		$type = (!$quote) ? 'invoice' : 'quote';

		$default_template = $this->mdl_ba_client_data->get($client_id, 'default_' . $type . '_template');

		if (!$default_template) {

			$default_template = $this->mdl_ba_data->setting('default_' . $type . '_template');
	...
	function jquery_client_invoice_group($client_id, $type = 'invoice') {
		...
		$default_group_id = $this->mdl_ba_client_data->get($client_id, 'default_' . $type . '_group_id');

		if (!$default_group_id) {

			$default_group_id = $this->mdl_ba_data->setting('default_' . $type . '_group_id');

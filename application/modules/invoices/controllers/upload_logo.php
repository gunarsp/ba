...
class Upload_Logo extends Admin_Controller {
	...
	function index() {
		...
		if ($this->input->post('btn_upload_logo')) {
			...
			if (!$this->upload->do_upload()) {
				...
				$this->mdl_ba_data->save('invoice_logo', $upload_data['file_name']);
	...
	function delete() {
		...
		if ($this->mdl_ba_data->setting('invoice_logo') == uri_assoc('invoice_logo', 4)) {

			$this->mdl_ba_data->delete('invoice_logo');

...
class Client_Center extends Client_Center_Controller {
	...
	public function index() {
		...
		$params = array(
			'open'		=> array(
				'limit'	=>	10,
				'where'	=>	array(
					'ba_invoice_statuses.invoice_status_type'		=>	1,
					'ba_invoices.client_id'						=>	$this->session->userdata('client_id'),
			...
			'pending'	=>	array(
				'limit'	=>	10,
				'where'	=>	array(
					'ba_invoice_statuses.invoice_status_type'		=>	2,
					'ba_invoices.client_id'						=>	$this->session->userdata('client_id'),
					'ba_invoices.invoice_is_quote'					=>	0
				),
				'set_client'	=>	TRUE
			...
			'closed'	=>	array(
				'limit'	=>	10,
				'where'	=>	array(
					'ba_invoice_statuses.invoice_status_type'		=>	3,
					'ba_invoices.client_id'						=>	$this->session->userdata('client_id'),
					'ba_invoices.invoice_is_quote'					=>	0
				),
				'set_client'	=>	TRUE
	...
	public function _get_invoice_template() {
		...
		$invoice_template = $this->mdl_ba_client_data->get($this->session->userdata('client_id'), 'default_invoice_template');

		if (!$invoice_template) {

			$invoice_template = $this->mdl_ba_data->setting('default_invoice_template');

...
class Invoice_Mailer extends Admin_Controller {
    ...
    function form() {
		...
		$default_template = $this->mdl_email_templates->get_by_id($this->mdl_ba_data->setting('default_invoice_email_template'));
        ...
        if (!$this->mdl_mailer->validate_invoice_email()) {
            ...
            if (!$_POST) {
                ...
                $this->mdl_mailer->set_form_value('email_cc', $this->mdl_ba_data->setting('default_cc'));
                $this->mdl_mailer->set_form_value('email_bcc', $this->mdl_ba_data->setting('default_bcc'));
    ...
    function overdue() {
        ...
        if (!$this->mdl_mailer->validate_invoice_overdue()) {
            ...
            if (!$_POST) {
				...
				$default_template = $this->mdl_email_templates->get_by_id($this->mdl_ba_data->setting('default_overdue_invoice_email_template'));
				$email_body = ($default_template) ? $default_template->email_template_body : ' ';
				$email_footer = ($default_template) ? $default_template->email_template_footer : ' ';
                ...
                $this->mdl_mailer->set_form_value('invoice_template', $this->mdl_ba_data->setting('default_invoice_template'));

...
class Mdl_Tax_Rates extends MY_Model {
	...
	public function __construct() {
		...
		$this->table_name = 'ba_tax_rates';

		$this->primary_key = 'ba_tax_rates.tax_rate_id';

		$this->select_fields = "
		SQL_CALC_FOUND_ROWS ba_tax_rates.*";
	...
	public function delete($tax_rate_id) {
		...
		*/
		...
		$query = $this->db->get('ba_invoice_tax_rates');
		...
		elseif ($this->mdl_ba_data->setting('default_tax_rate_id') == $tax_rate_id) {
	...
	public function get_invoice_tax_rates($invoice_id) {
		...
		$this->db->order_by('ba_invoice_tax_rates.invoice_tax_rate_id');

		return $this->db->get('ba_invoice_tax_rates')->result();

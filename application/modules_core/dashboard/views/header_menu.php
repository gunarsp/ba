...
<?php if ($this->session->userdata('global_admin') and $this->mdl_ba_modules->num_custom_modules_enabled) { ?>

<li><?php echo anchor('ba_modules', $this->lang->line('custom_modules')); ?>
	<ul>
		<?php $x = 0; foreach ($this->mdl_ba_modules->custom_modules as $module) {;?>
			<?php if ($module->module_enabled) { $x++;?>
				<li <?php if ($x == $this->mdl_ba_modules->num_custom_modules_enabled) { ?>class="last"<?php } ?>>

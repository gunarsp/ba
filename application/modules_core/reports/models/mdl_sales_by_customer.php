...
class Mdl_Sales_by_Customer extends CI_Model {
	...
	public function get($from_date = 0, $to_date = 0) {
		...
		$this->db->select('
			...
			SUM(ba_invoice_amounts.invoice_item_subtotal) AS amt_sales,
			SUM(ba_invoice_amounts.invoice_total) AS amt_sales_inc_tax,
			COUNT(ba_invoices.invoice_id) AS num_invoices');
		...
		if (!$this->session->userdata('global_admin')) {
			...
			$this->db->where('ba_invoices.user_id', $this->session->userdata('user_id'));

		}

		$this->db->group_by('ba_clients.client_id');

		$this->db->order_by('ba_clients.client_name');

		return $this->db->get('ba_invoices')->result();

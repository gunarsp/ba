	...
	function form() {
		...
		if (!$this->mdl_items->validate()) {
			...
			elseif (!$_POST AND !uri_assoc('invoice_item_id', 4)) {
				...
				$default_item_tax_rate_id = $this->mdl_ba_userdata->setting('default_item_tax_rate_id');
				$default_item_tax_option = $this->mdl_ba_userdata->setting('default_item_tax_option');

				if (!$default_item_tax_rate_id) {

					$default_item_tax_rate_id = $this->mdl_ba_data->setting('default_item_tax_rate_id');

				}

				if ($default_item_tax_option == "") {
					
					$default_item_tax_option = $this->mdl_ba_data->setting('default_item_tax_option');
			...
			$invoice = $this->mdl_invoices->get(array('where'=>array('ba_invoices.invoice_id'=>uri_assoc('invoice_id', 4))));

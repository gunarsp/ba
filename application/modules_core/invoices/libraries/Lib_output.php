...
class Lib_output {
	...
	function pdf($invoice_id, $invoice_template) {
		...
		$this->CI->load->helper($this->CI->mdl_ba_data->setting('pdf_plugin'));
	...
	function payment_link($invoice) {
		...
		if ($this->CI->mdl_ba_data->setting('merchant_enabled')) {

			$merchant_driver = strtolower($this->CI->mdl_ba_data->setting('merchant_driver'));
			...
			$params = array(
				'test_mode'				=>	FALSE,
				'merchant_account_id'	=>	$this->CI->mdl_ba_data->setting('merchant_account_id'),
				'amount'				=>	$invoice->invoice_balance,
				'currency_code'			=>	$this->CI->mdl_ba_data->setting('merchant_currency_code'),

...
class Mdl_Invoice_Amounts extends CI_Model {
    ...
    /**
     * TABLE: ba_invoice_tax_rates	Global taxes
     * invoice_tax_rate_id
     * invoice_id
     * tax_rate_id
    ...
    /**
     * TABLE: ba_invoice_amounts
     * invoice_amount_id
     * invoice_id
     * invoice_item_subtotal	Sum of ba_invoice_item_amounts.item_total
     * invoice_item_tax			Sum of ba_invoice_item_amounts.item_tax
     ...
     * invoice_paid				Sum of ba_payments.payment_amount
    ...
    function _adjust($invoice_id) {
        ...
        $invoice_items = $this->db->get('ba_invoice_items')->result();
    ...
    function _update_invoice_item_amounts($item) {
        ...
        /* Calculations for ba_invoice_item_amounts table */
        ...
        if ($this->db->get('ba_invoice_item_amounts')->num_rows()) {

            $this->db->where('invoice_item_id', $item->invoice_item_id);
            $this->db->update('ba_invoice_item_amounts', $db_array);

        }

        else {

            $this->db->insert('ba_invoice_item_amounts', $db_array);
    ...
    function _update_invoice_amounts($invoice_id) {
        ...
        /* Calculations for ba_invoice_amounts table */
        ...
        $this->db->select(
            ...
            'IFNULL((SELECT SUM(payment_amount) FROM ba_payments WHERE invoice_id = ' . $invoice_id . '),0.00) AS invoice_paid',
            FALSE
        );

        $this->db->join('ba_invoice_items', 'ba_invoice_items.invoice_item_id = ba_invoice_item_amounts.invoice_item_id');

        $this->db->where('invoice_id', $invoice_id);

        $invoice_amounts = $this->db->get('ba_invoice_item_amounts')->result();
        ...
        foreach ($invoice_amounts as $invoice_amount) {
            ...
            if ($this->db->get('ba_invoice_amounts')->num_rows()) {
                ...
                $this->db->insert('ba_invoice_amounts', $db_array);
    ...
    function _update_invoice_status($invoice_id) {
        ...
        $params = array(
            'where'	=>	array(
                'ba_invoices.invoice_id'	=>	$invoice_id
            )
        );
        ...
        if ($invoice->invoice_balance > 0) {
            ...
            if ($invoice->invoice_status_type <> 3 and $invoice->invoice_total > 0) {
                ...
                $db_array = array(
                    'invoice_status_id'	=>	$this->mdl_ba_data->setting('default_closed_status_id')
                );
    ...
    function _update_invoice_taxes($invoice_id) {
        ...
        $invoice_item_taxable = $this->db->get('ba_invoice_item_amounts')->row()->invoice_item_taxable;
        ...
        $this->db->join('ba_invoice_amounts', 'ba_invoice_amounts.invoice_id = ba_invoice_tax_rates.invoice_id');
        $this->db->join('ba_tax_rates', 'ba_tax_rates.tax_rate_id = ba_invoice_tax_rates.tax_rate_id');
        $this->db->where('ba_invoice_tax_rates.invoice_id', $invoice_id);
        ...
        foreach ($invoice_tax_rates as $rate) {
            ...
            if ($rate->tax_rate_option == 1) {
                ...
                $this->db->update('ba_invoice_tax_rates', $db_array);
            ...
            $this->db->where('invoice_id', $invoice_id);
            $invoice_tax = $this->db->get('ba_invoice_tax_rates')->row()->invoice_tax;

...
class Clients extends Admin_Controller {
	...
	public function index() {
		...
		switch ($order_by) {
			case 'client_id':
				$params['order_by'] = 'ba_clients.client_id ' . $order;
				break;
			case 'client_name':
				$params['order_by'] = 'ba_clients.client_name ' . $order;
				break;
			case 'credit_amount':
			...
			case 'balance':
				...
				break;
			case 'client_email':
				$params['order_by'] = 'ba_clients.client_email_address ' . $order;
				break;
			case 'client_phone':
				$params['order_by'] = 'ba_clients.client_phone_number ' . $order;
				break;
			case 'client_active':
				$params['order_by'] = 'ba_clients.client_active ' . $order;
				break;
			default:
				$params['order_by'] = 'ba_clients.client_name';
		}
		...
		if ($this->mdl_clients->validate()) {
			...
			foreach ($this->input->post('client_settings') as $key=>$value) {
				...
				if ($value) {
					...
					$this->mdl_ba_client_data->save($client_id, $key, $value);

				}

				else {

					$this->mdl_ba_client_data->delete($client_id, $key);
			...
			if ($client_id) {
				...
				$this->mdl_ba_client_data->set_session_data($client_id);

			}

			$client_params = array(
				'where'	=>	array(
					'ba_clients.client_id'	=>	$client_id
				)
			);

			$contact_params = array(
				'where'	=>	array(
					'ba_contacts.client_id'    =>  $client_id
				)
			);

			$invoice_params = array(
				'where'	=>	array(
					'ba_invoices.client_id'        =>	$client_id,
					'ba_invoices.invoice_is_quote' =>  0
				)
			);
			...
			$quote_params = array(
				'where'	=>	array(
					'ba_invoices.client_id'		=>	$client_id,
					'ba_invoices.invoice_is_quote'	=>	1
				)
			);

			if (!$this->session->userdata('global_admin')) {

				$invoice_params['where']['ba_invoices.user_id'] = $this->session->userdata('user_id');
	...
	public function jquery_lookup() {
		...
		$clients = $this->db->get('ba_clients')->result();

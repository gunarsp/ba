...
function phpmail_send($from, $to, $subject, $message, $attachment_path = NULL, $cc = NULL, $bcc = NULL) {
    ...
    if ($CI->mdl_ba_data->setting('email_protocol') == 'smtp') {

        $mail->IsSMTP();

        $mail->SMTPAuth = true;

        if ($CI->mdl_ba_data->setting('smtp_security')) {

            $mail->SMTPSecure = $CI->mdl_ba_data->setting('smtp_security');

        }

        $mail->Host = $CI->mdl_ba_data->setting('smtp_host');

        $mail->Port = $CI->mdl_ba_data->setting('smtp_port');

        $mail->Username = $CI->mdl_ba_data->setting('smtp_user');

        $mail->Password = $CI->mdl_ba_data->setting('smtp_pass');

    }

    elseif ($CI->mdl_ba_data->setting('email_protocol') == 'sendmail') {

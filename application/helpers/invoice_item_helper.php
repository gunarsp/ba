...
function invoice_item_tax_rate($item) {
    ...
    return format_number($item->tax_rate_percent, TRUE, $CI->mdl_ba_data->setting('decimal_taxes_num')) . '%';
...
function invoice_item_tax_sum_name($sum) {
    ...
    return $sum->tax_rate_name . ' @ ' . format_number($sum->tax_rate_percent, TRUE, $CI->mdl_ba_data->setting('decimal_taxes_num')) . '%';
...
function format_qty($qty) {
    ...
    return ($CI->mdl_ba_data->setting('display_quantity_decimals')) ? format_number($qty) : format_number($qty, TRUE, 0);

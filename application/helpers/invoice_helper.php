...
function invoice_payment_link($invoice) {
	...
	if ($CI->mdl_ba_data->setting('merchant_enabled')) {
...
function invoice_date_entered($invoice) {
	...
	return date($CI->mdl_ba_data->setting('default_date_format'), $invoice->invoice_date_entered);
...
function invoice_due_date($invoice) {
	...
	return date($CI->mdl_ba_data->setting('default_date_format'), $invoice->invoice_due_date);
...
function invoice_logo($output_type = 'pdf') {
	...
	if ($CI->mdl_ba_data->setting('include_logo_on_invoice') == 'TRUE' AND $CI->mdl_ba_data->setting('invoice_logo')) {

		if ($output_type == 'pdf') {

			/** Use a system path to include the image in the PDF **/
			return '<img src="' . getcwd() . '/uploads/invoice_logos/' . $CI->mdl_ba_data->setting('invoice_logo') . '" />';
		...
		elseif ($output_type == 'html') {
			...
			return '<img src="' . base_url() . 'uploads/invoice_logos/' . $CI->mdl_ba_data->setting('invoice_logo') . '" />';

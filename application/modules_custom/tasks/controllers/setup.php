...
class Setup extends Admin_Controller {
	...
	function install() {
		...
		$queries = array(
			...
			"CREATE TABLE IF NOT EXISTS `ba_tasks_invoices` (
	...
	function uninstall() {
		...
		$queries = array(
			"DROP TABLE IF EXISTS `ba_tasks`",
			"DROP TABLE IF EXISTS `ba_tasks_invoices`"
		);
	...
	function upgrade() {
		...
		$installed_version = $this->mdl_ba_modules->custom_modules['tasks']->module_version;
